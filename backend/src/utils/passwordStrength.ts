 ...
 * 
 * Provides password strength checking with levels:
 * - WEAK: Score 0-1 (should be rejected)
 * - FAIR: Score 2 (should be rejected)
 * - GOOD: Score 3 (acceptable)
 * - STRONG: Score 4+ (preferred)
 * 
 * Also includes common password detection
 */
 ...
 * 
 * Scoring criteria:
 * - Length: 0-2 points (8+ chars = 1, 12+ chars = 2)
 * - Lowercase: 0-1 point
 * - Uppercase: 0-1 point
 * - Numbers: 0-1 point
 * - Special chars: 0-1 point
 * - Common password: -2 penalty
 * 
 * Total: 0-4 points
 */
const calculateScore = (password: string): { score: number; feedback: string[] } => {
  ...
  // Penalty for common passwords (applied after all other checks)
...
/**
 * Check if password meets minimum requirements (GOOD or STRONG)
 * 
 * @param password - Password to check
 * @returns true if password is GOOD or STRONG
 */
export const isPasswordStrongEnough = (password: string): boolean => {
  const result = checkPasswordStrength(password);
  return result.strength === PasswordStrength.GOOD || result.strength === PasswordStrength.STRONG;
};
